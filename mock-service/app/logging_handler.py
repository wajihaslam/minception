"""
logging_handler.py — DB-backed logging for mock-service.

Minimal duplicate of admin-service/app/logging_handler.py.
Note: no JWT extraction (mock-service has no auth layer).
Both services write to the same error_logs collection via ADMIN_DB_URL.

TODO: consolidate into a shared package when the monorepo adds one.
"""
import logging
import queue
import re
...
class RequestContextFilter(logging.Filter):
    def filter(self, record: logging.LogRecord) -> bool:
        ...
        record.req_user_id = None  # mock-service has no auth
...
class DBLogHandler(logging.Handler):
    ...
    Uses ADMIN_DB_URL so mock-service writes to the shared admin DB.
    """
