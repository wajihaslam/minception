...
export function useCopyEndpoint(gatewayId: string) {
  const qc = useQueryClient()
  return useMutation({
    mutationFn: async (eid: string) => {
      const { data } = await apiClient.post<ApiResponse<Endpoint>>(
        `/admin/gateways/${gatewayId}/endpoints/${eid}/copy`,
      )
      if (!data.success || !data.data) throw new Error(data.error?.message ?? 'Failed to copy endpoint')
      return data.data
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: gatewayKey(gatewayId) }),
  })
